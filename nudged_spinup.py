 ...
 for yy in range(begy,endy+1): #Loop over the spinup years
  ...
  for var in restart.keys():
   ...
   if (loc_pft==1)&(len(dim_var)==4):
    ...
    restart[var].values[:,:,:,id_pft,id_lat,id_lon]=np.copy(ncf[:,:,:,id_pft,id_lat,id_lon])
 os.system("rm -f "+homedir+"/"+name_restart[rr]+".nc")
 os.system("rm -f "+homedir+"/"+name_restart[rr]+"_i.nc")
 
 restart.to_netcdf(homedir+"/"+name_restart[rr]+".nc") 
 os.system("cp -r "+file_restart+ " "+homedir+"/"+name_restart[rr]+"_i.nc") 
...
for yy in range(begy,endy+1):
  ...
  for var in list_vars:
   ...
   if (loc_pft==1)&(len(dim_var)==4):
    ...
    restart[var].values[:,:,:,id_pft,id_lat,id_lon]=np.copy(ncf[:,:,:,id_pft,id_lat,id_lon])
    print(var)
os.system("rm -f "+homedir+"/output_f.nc")
os.system("rm -f "+homedir+"/output_i.nc")
    
restart.to_netcdf(homedir+"/output_f.nc")
os.system("cp -r "+file_output+ " "+homedir+"/output_i.nc")
